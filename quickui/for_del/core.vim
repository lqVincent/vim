...
" global variables
"----------------------------------------------------------------------
let g:preview#core#has_nvim = has('nvim')
let g:preview#core#has_popup = exists('*popup_create') && v:version >= 800
let g:preview#core#has_floating = has('nvim-0.4')
...
" object pool acquire
"----------------------------------------------------------------------
function! preview#core#object_acquire(name)
	if !exists('g:preview#core#__object_pool__')
		let g:preview#core#__object_pool__ = {}
	endif
	if !has_key(g:preview#core#__object_pool__, a:name)
		let g:preview#core#__object_pool__[a:name] = []
	endif
	let array = g:preview#core#__object_pool__[a:name]
	if len(array) == 0
		return v:null
	endif
...
" object pool release
"----------------------------------------------------------------------
function! preview#core#object_release(name, obj)
	if !exists('g:preview#core#__object_pool__')
		let g:preview#core#__object_pool__ = {}
	endif
	if !has_key(g:preview#core#__object_pool__, a:name)
	...
	call add(g:preview#core#__object_pool__[a:name], a:obj)
endfunc
...
" replace string
"----------------------------------------------------------------------
function! preview#core#string_replace(text, old, new)
...
" compose two string
"----------------------------------------------------------------------
function! preview#core#string_compose(target, pos, source)
	if a:source == ''
		return a:target
	endif
	let pos = a:pos
...
" fit size
"----------------------------------------------------------------------
function! preview#core#string_fit(source, size)
	let require = a:size
	let source = a:source
	let size = len(source)
	if size <= require
...
"----------------------------------------------------------------------
function! preview#core#expand_text(string) abort
	let partial = []
	let index = 0
	while 1
...
"----------------------------------------------------------------------
function! preview#core#escape(text)
	let text = a:text
	let rest = ''
	let start = 0
	let obj = ['', '', -1, -1, -1]
...
" list parse
"----------------------------------------------------------------------
function! preview#core#single_parse(description)
	...
	for text in split(text, "\t")
		let obj = preview#core#escape(text)
		let item.part += [obj[0]]
...
" tabpage instance
"----------------------------------------------------------------------
function! preview#core#instance()
	if exists('t:__tools__')
		return t:__tools__
	endif
	let t:__tools__ = {}
	return t:__tools__
...
" buffer instance
"----------------------------------------------------------------------
function! preview#core#object(bid)
	let name = '__tools__'
...
"----------------------------------------------------------------------
function! preview#core#popup_alloc(name)
	let inst = preview#core#instance()
...
"----------------------------------------------------------------------
function! preview#core#popup_release(name, winid)
	let inst = preview#core#instance()
	if !has_key(inst, 'popup_cache')
		let inst.popup_cache = {}
	endif
...
" local object
"----------------------------------------------------------------------
function! preview#core#popup_local(winid)
	let inst = preview#core#instance()
...
" erase local data
"----------------------------------------------------------------------
function! preview#core#popup_clear(winid)
	let inst = preview#core#instance()
	if !has_key(inst, 'popup_local')
		let inst.popup_local = {}
	endif
...
" vim/nvim compatible
"----------------------------------------------------------------------
function! preview#core#win_execute(winid, command)
	if g:preview#core#has_popup != 0
...
" get a named buffer
"----------------------------------------------------------------------
function! preview#core#scratch_buffer(name, textlist)
...
" dummy filter
"----------------------------------------------------------------------
function! preview#core#mock_function(id, text)
	return 0
endfunc
...
" highlight region
"----------------------------------------------------------------------
function! preview#core#high_region(name, srow, scol, erow, ecol, virtual)
...
" patterns
"----------------------------------------------------------------------
function! preview#core#border_extract(pattern)
	let parts = ['', '', '', '', '', '', '', '', '', '', '']
	for idx in range(11)
...
function! preview#core#border_convert(pattern)
	if type(a:pattern) == v:t_string
		let p = preview#core#border_extract(a:pattern)
	else
		let p = a:pattern
	endif
...
let s:border_styles[1] = preview#core#border_extract('+-+|-|+-+++')
let s:border_styles[2] = preview#core#border_extract('┌─┐│─│└─┘├┤')
let s:border_styles[3] = preview#core#border_extract('╔═╗║─║╚═╝╟╢')
let s:border_styles[4] = preview#core#border_extract('/-\|-|\-/++')

let s:border_ascii = preview#core#border_extract('+-+|-|+-+++')

function! preview#core#border_install(name, pattern)
	let s:border_styles[a:name] = preview#core#border_extract(a:pattern)
endfunc

function! preview#core#border_get(name)
...
function! preview#core#border_vim(name)
	let border = preview#core#border_get(a:name)
	return preview#core#border_convert(border)
endfunc
...
"----------------------------------------------------------------------
function! preview#core#cursor_pos()
...
"----------------------------------------------------------------------
function! preview#core#in_screen(line, column, width, height)
	let x = a:column - 1
	let y = a:line - 1
	let w = a:width
	let h = a:height
...
" window fit screen
"----------------------------------------------------------------------
function! preview#core#screen_fit(line, column, width, height)
	let x = a:column - 1
	let y = a:line - 1
	let w = a:width
	let h = a:height
...
" fit screen
"----------------------------------------------------------------------
function! preview#core#around_cursor(width, height)
	let cursor_pos = preview#core#cursor_pos()
	let row = cursor_pos[0] + 1
	let col = cursor_pos[1] + 1
	if preview#core#in_screen(row, col, a:width, a:height)
		return [row, col]
	endif
	...
	return preview#core#screen_fit(row, col, a:width, a:height)
endfunc
...
" safe input
"----------------------------------------------------------------------
function! preview#core#input(prompt, text)
	call inputsave()
	try
...
" safe change dir
"----------------------------------------------------------------------
function! preview#core#chdir(path)
	if has('nvim')
...
" full file name
"----------------------------------------------------------------------
function! preview#core#fullname(f)
	let f = a:f
	if f =~ "'."
		try
			redir => m
...
"----------------------------------------------------------------------
function! preview#core#find_root(name, markers, strict)
...
" find project root
"----------------------------------------------------------------------
function! preview#core#project_root(name, ...)
	let markers = ['.project', '.git', '.hg', '.svn', '.root']
	if exists('g:tools_rootmarks')
		let markers = g:tools_rootmarks
	elseif exists('g:asyncrun_rootmarks')
		let markers = g:asyncrun_rootmarks
	endif
	let path = preview#core#fullname(a:name)
	let strict = (a:0 > 0)? (a:1) : 0
	return preview#core#find_root(path, markers, strict)
endfunc
...
" expand macros
"----------------------------------------------------------------------
function! preview#core#expand_macros()
	let macros = {}
	...
	let macros['VIM_ROOT'] = preview#core#project_root('%', 0)
...
"----------------------------------------------------------------------
function! preview#core#write_script(command, pause)
	let tmpname = fnamemodify(tempname(), ':h') . '\tools1.cmd'
	...
	if s:windows != 0
		...
		let tmpname = fnamemodify(tempname(), ':h') . '/tools1.sh'
	endif
