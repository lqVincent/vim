...
function! s:get_tools_width()
	let width = get(g:, 'tools_tools_width', 70)
endfunc
...
"----------------------------------------------------------------------
function! preview#preview#buffer_switch(bid)
	let code = g:preview#listbox#current.tag
...
" get content
"----------------------------------------------------------------------
function! preview#preview#list_buffer(switch)
	...
	for bid in bids
		...
		let cmd = 'call preview#preview#buffer_switch(' . bid . ')'
		if a:switch != ''
	...
	let opts.border = g:preview#style#border
	let opts.keymap = {}
	...
	if exists('g:tools_tools_width')
		let opts.w = preview#utils#tools_width()
	endif
	" let opts.syntax = 'cpp'
	...
	call preview#listbox#open(content, opts)
endfunc
...
" list function
"----------------------------------------------------------------------
function! preview#preview#list_function()
	let ctags = get(g:, 'tools_ctags_exe', 'ctags')
	if !executable(ctags)
		let msg = 'Not find ctags, add to $PATH or specify in '
		call preview#utils#errmsg(msg . 'g:tools_ctags_exe')
		return -1
	endif
	let items = preview#tags#function_list(bufnr(), &ft)
	if len(items) == 0
		call preview#utils#errmsg('No content !')
		return -2
	endif
	let content = []
	let cursor = -1
	...
	let maxheight = (&lines) * 60 / 100
	let maxwidth = 0
	let indents = get(g:, 'tools_tags_indent', {})
	for item in items
		if ln >= item.line
			let cursor = index
		endif
...
"----------------------------------------------------------------------
function! preview#preview#list_register()
endfunc
...
"----------------------------------------------------------------------
function! preview#preview#python_help(word)
	let python = get(g:, 'tools_tools_python', '')
	if python == ''
	...
	let opts.tabstop = 12
	call preview#textbox#command(cmd, opts)
endfunc
...
" display messages
"----------------------------------------------------------------------
function! preview#preview#display_messages()
	let x = ''
	redir => x
	silent! messages
	redir END
	...
	if len(content) == 0
		call preview#utils#errmsg('Empty messages')
		return -1
	endif
	...
	call preview#textbox#open(content, opts)
endfunc
...
" preview quickfix
"----------------------------------------------------------------------
function! preview#preview#preview_quickfix(...)
	if preview#preview#visible()
		call preview#preview#close()
		return 0
	endif
	if &bt != 'quickfix'
		call preview#utils#errmsg('Not in quickfix window !')
		return -1
	endif
	if !exists('b:__tools_qf__')
		let b:__tools_qf__ = {}
	endif
	let obj = b:__tools_qf__
	...
	if index < 1 || index > len(obj.items)
		call preview#utils#errmsg('No information in this line')
		return -2
	endif
	...
	call preview#preview#open(name, opts)
...
" preview tag
"----------------------------------------------------------------------
function! preview#preview#preview_tag(tagname)
	let tagname = (a:tagname == '')? expand('<cword>') : a:tagname
	if tagname == ''
		call preview#utils#errmsg('Error: empty tagname')
		return 0
	endif
	let obj = preview#core#object(0)
	let reuse = 0
	if has_key(obj, 'ptag')
		let ptag = obj.ptag
	...
	if reuse == 0
		let obj.ptag = {}
		let ptag = obj.ptag
		let ptag.taglist = preview#tags#tagfind(tagname)
	...
	if len(ptag.taglist) == 0
		call preview#utils#errmsg('E257: preview: tag not find "' . tagname . '"')
		return 1
	endif
	...
	if !filereadable(filename)
		call preview#utils#errmsg('E484: Can not open file '.filename)
		return 2
	endif
	if !has_key(taginfo, 'line')
		call preview#utils#errmsg('Error: no "line" information in your tags, regenerate with -n')
		return 3
	endif
	let text = '('.(ptag.index + 1).'/'.len(ptag.taglist).')'
	let opts = {'cursor':taginfo.line, 'title':text}
	call preview#preview#open(filename, opts)
	...
	let display = get(g:, 'tools_preview_tag_msg', display)
	if display != 0
		call preview#utils#print(text, 1)
	endif
	return 0
endfunc
...
"----------------------------------------------------------------------
function! preview#preview#display_help(tag)
	...
	if !exists('s:help_tags')
		call preview#utils#errmsg('Sorry, not find help tags in $VIMRUNTIME')
		return -1
	endif
	if !has_key(s:help_tags, a:tag)
		call preview#utils#errmsg('E149: Sorry, no help for '. a:tag)
		return -2
	endif
	...
	if !filereadable(name)
		call preview#utils#errmsg('E484: Sorry, cannot open file '.name)
		return -3
	endif
	let content = readfile(name)
	let opts = {'syntax':'help', 'color':'QuickPreview', 'close':'button'}
	let opts.title = 'Help: ' . fnamemodify(name, ':t')
	let g:preview#preview#hint = item[1]
	let opts.command = ['silent! exec g:preview#preview#hint']
	let opts.command += ["exec 'nohl'"]
	let opts.command += ["normal zz"]
	let opts.w = 80
	" echom opts
	let winid = preview#textbox#open(content, opts)
	return 0
endfunc
...
function! s:remember_cursor_context(code)
	let hwnd = g:preview#context#current
	let name = hwnd.opts.keep_name
	let s:previous_cursor[name] = g:preview#context#cursor
endfunc

function! s:remember_cursor_listbox(code)
	let hwnd = g:preview#listbox#current
	let name = hwnd.opts.keep_name
	let s:previous_cursor[name] = g:preview#listbox#cursor
endfunc

function! preview#preview#clever_context(name, content, opts)
	...
	let content = preview#context#reduce_items(a:content)
	call preview#context#open(content, opts)
endfunc

function! preview#preview#clever_listbox(name, content, opts)
	let opts = deepcopy(a:opts)
	let opts.index = get(s:previous_cursor, a:name, -1)
	let opts.keep_name = a:name
	let opts.callback = function('s:remember_cursor_listbox')
	call preview#listbox#open(a:content, opts)
endfunc
...
" terminal
"----------------------------------------------------------------------
function! preview#preview#terminal(name)
	if !exists('g:tools_terminal_tools')
		let g:tools_terminal_tools = {}
	endif
	if !has_key(g:tools_terminal_tools, a:name)
		call preview#utils#errmsg('ERROR: tool ' . a:name . ' not defined !')
		return -1
	endif
	let tools = g:tools_terminal_tools[a:name]
	if !has_key(tools, 'cmd')
		call preview#utils#errmsg('ERROR: key cmd not present in tool ' . a:name)
		return -1
	endif
	let opts = {}
	let cmd = tools.cmd
	let w = get(g:, 'tools_terminal_w', 80)
	let h = get(g:, 'tools_terminal_h', 24)
	...
	call preview#terminal#dialog(cmd, opts)
	return 0
endfunc
