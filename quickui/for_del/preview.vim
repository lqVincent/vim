...
"----------------------------------------------------------------------
function! preview#display(content, opts)
	call preview#close()
	if type(a:content) == v:t_string && filereadable(a:content) == 0
		call preview#utils#errmsg('E212: Can not open file: '. a:content)
		return -1
	endif
	...
	let w = (w < 0)? 50 : w
	let h = (h < 0)? 30 : h
	let border = get(a:opts, 'border', g:preview#style#border)
	...
	if has('nvim') == 0
		...
		let opts.borderchars = preview#core#border_vim(border)
		...
		if type(source) == v:t_number
			...
			let bid = preview#core#scratch_buffer('preview', source)
		endif
		...
		if border > 0 && get(g:, 'tools_nvim_simulate_border', 1) != 0
			let back = preview#utils#make_border(w, h, border, title, button)
			let nbid = preview#core#scratch_buffer('previewborder', back)
	...
	call setbufvar(winbufnr(winid), '__tools_cursor__', cursor)
	call preview#core#win_execute(winid, cmdlist)
	call preview#utils#update_cursor(winid)
...
"----------------------------------------------------------------------
function! preview#scroll(offset)
	if s:private.state != 0
		let winid = s:private.winid
		if s:private.winid >= 0
			noautocmd call preview#utils#scroll(winid, a:offset)
			noautocmd call preview#utils#update_cursor(winid)
		endif
	endif
endfunc
...
" preview file
"----------------------------------------------------------------------
function! preview#open(content, opts)
	call preview#close()
	if type(a:content) == v:t_string && filereadable(a:content) == 0
		call preview#utils#errmsg('E484: Cannot open file ' . a:content)
		return -1
	endif
	let opts = {}
	let opts.w = get(g:, 'tools_preview_w', g:preview#style#preview_w)
	let opts.h = get(g:, 'tools_preview_h', g:preview#style#preview_h)
	let opts.number = get(a:opts, 'number', g:preview#style#preview_number)
	...
	if g:preview#style#preview_bordercolor != ''
		let opts.bordercolor = g:preview#style#preview_bordercolor
	endif
	let opts.persist = get(a:opts, 'persist', 0)
	let opts.focusable = get(g:, 'tools_preview_focusable', 1)
	if has_key(a:opts, 'syntax')
		let opts.syntax = a:opts.syntax
	endif
	let hr = preview#display(a:content, opts)
