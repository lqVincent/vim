...
"----------------------------------------------------------------------
function! preview#tags#taglist(pattern)
    let ftags = []
    try
...
" easy tagname
"----------------------------------------------------------------------
function! preview#tags#tagfind(tagname)
	let pattern = escape(a:tagname, '[\*~^')
	let result = preview#tags#taglist("^". pattern . "$")
...
" function signature
"----------------------------------------------------------------------
function! preview#tags#signature(funname, fn_only, filetype)
	let tags = preview#tags#tagfind(a:funname)
...
" get function list
"----------------------------------------------------------------------
function! preview#tags#ctags_function(bid, ft)
	let parameters = {
			\ "aspvbs": "--asp-kinds=f",
			\ "awk": "--awk-kinds=f",
	...
	if exists('g:tools_tags_list')
		let extras = get(g:tools_tags_list, ft, extras)
	endif
	let cmd = ctags . ' -n -u --fields=k ' . extras . ' -f- '
	let output = preview#utils#system(cmd . '"' . srcname . '"')
	if modified
		call delete(srcname)
	endif
	let items = []
...
" query function list
"----------------------------------------------------------------------
function! preview#tags#function_list(bid, ft)
	let changedtick = getbufvar(a:bid, 'changedtick')
	let currenttick = getbufvar(a:bid, '__tools_tags_tick', -100)
	let start = reltime()
	if currenttick != changedtick
		let items = preview#tags#ctags_function(a:bid, a:ft)
		call setbufvar(a:bid, '__tools_tags_func', items)
		call setbufvar(a:bid, '__tools_tags_tick', changedtick)
	endif
	let items = getbufvar(a:bid, '__tools_tags_func')
	if type(items) == v:t_string
		let items = preview#tags#ctags_function(a:bid, a:ft)
		call setbufvar(a:bid, '__tools_tags_func', items)
		call setbufvar(a:bid, '__tools_tags_tick', changedtick)
	endif
	let g:preview#tags#elapse = reltimestr(reltime(start))
	" echo g:preview#tags#elapse
	return items
endfunc
