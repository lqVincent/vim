...
"----------------------------------------------------------------------
function! preview#utils#item_parse(description)
	...
	if text =~ '^-\+$'
		...
		let text = preview#core#expand_text(text)
		let obj.enable = 1
		if strpart(text, 0, 1) == '~'
...
" alignment
"----------------------------------------------------------------------
function! preview#utils#context_align(item, left_size, right_size)
	let obj = a:item
...
" style: default
"----------------------------------------------------------------------
function! preview#utils#highlight(style)
...
"----------------------------------------------------------------------
function! preview#utils#system(cmd)
	let hr = s:python_system(a:cmd, get(g:, 'tools_python', 0))
	let g:preview#utils#shell_error = s:shell_error
	return hr
endfunc
...
" display a error msg
"----------------------------------------------------------------------
function! preview#utils#errmsg(what)
	redraw
	echohl ErrorMsg
	echom a:what
	echohl None
...
" safe print
"----------------------------------------------------------------------
function! preview#utils#print(content, highlight, ...)
...
" max height
"----------------------------------------------------------------------
function! preview#utils#max_height(percentage)
...
" cursor movement
"----------------------------------------------------------------------
function! preview#utils#movement(offset)
...
" cursor scroll
"----------------------------------------------------------------------
function! preview#utils#scroll(winid, offset)
	if type(a:offset) == v:t_number
		let cmd = 'call preview#utils#movement(' . a:offset . ')'
		call preview#core#win_execute(a:winid, cmd)
	else
		let cmd = 'call preview#utils#movement("' . a:offset . '")'
		call preview#core#win_execute(a:winid, cmd)
	endif
...
" centerize
"----------------------------------------------------------------------
function! preview#utils#center(winid)
	...
	let hr = preview#core#screen_fit(opts.line, opts.col, w, h)
	let opts.col = hr[1]
	let opts.line = hr[0]
	if g:preview#core#has_nvim == 0
...
"----------------------------------------------------------------------
function! preview#utils#show_cursor(winid, row)
	let height = winheight(0)
	let winline = winline()
	let curline = line('.')
	let topline = curline - winline + 1
	let topline = (topline < 1)? 1 : topline
	let botline = topline + height - 1
	let w:__tools_line__ = get(w:, '__tools_line__', -1)
	if a:row >= topline && a:row <= botline
		exec ":" . a:row
		if w:__tools_line__ != 1
			if g:preview#core#has_nvim == 0
				call popup_setoptions(a:winid, {'cursorline': 1})
			else
				call preview#core#win_execute(a:winid, 'setl cursorline')
			endif
		endif
		let w:__tools_line__ = 1
	else
		if w:__tools_line__ != 0
			if g:preview#core#has_nvim == 0
				call popup_setoptions(a:winid, {'cursorline': 0})
			else
				call preview#core#win_execute(a:winid, 'setl nocursorline')
			endif
		endif
		let w:__tools_line__ = 0
	endif
endfunc
...
" update cursor line
"----------------------------------------------------------------------
function! preview#utils#update_cursor(winid)
	let bid = winbufnr(a:winid)
	let row = getbufvar(bid, '__tools_cursor__', -1)
	let cmd = 'call preview#utils#show_cursor('. a:winid .', '.row.')'
	call preview#core#win_execute(a:winid, cmd)
endfunc
...
" get window line
"----------------------------------------------------------------------
function! preview#utils#get_cursor(winid)
	let g:preview#utils#__cursor_index__ = -1
	let cmd = 'let g:preview#utils#__cursor_index__ = line(".")'
	noautocmd call preview#core#win_execute(a:winid, cmd)
	return g:preview#utils#__cursor_index__
endfunc
...
"----------------------------------------------------------------------
function! preview#utils#current_topline()
	let height = winheight(0)
	let winline = winline()
	let curline = line('.')
	let topline = curline - winline + 1
...
"----------------------------------------------------------------------
function! preview#utils#get_topline(winid)
	let g:preview#utils#__cursor_topline__ = -1
	let cmd = 'let g:preview#utils#__cursor_topline__ = '
	let cmd = cmd . 'preview#utils#current_topline()'
	call preview#core#win_execute(a:winid, cmd)
	return g:preview#utils#__cursor_topline__
endfunc
...
" make border
"----------------------------------------------------------------------
function! preview#utils#make_border(width, height, border, title, ...)
	let pattern = preview#core#border_get(a:border)
	let image = []
	let w = a:width
	let h = a:height
	...
	let align = (a:0 > 1)? (a:2) : ''
	let text = image[0]
	let title = preview#core#string_fit(a:title, w)
	if align == '' || align == 'l'
		let text = preview#core#string_compose(text, 1, title)
	elseif align == 'm'
		let left = (w + 2 - len(title)) / 2
		let text = preview#core#string_compose(text, left, title)
	elseif align == 'r'
		let left = w + 2 - len(title) - 1
		let text = preview#core#string_compose(text, left, title)
	endif
	if button != 0
		let text = preview#core#string_compose(text, w + 1, 'X')
	endif
	let image[0] = text
	return image
endfunc
...
" search or jump
"----------------------------------------------------------------------
function! preview#utils#search_or_jump(winid, cmd)
	if a:cmd == '/' || a:cmd == '?'
		let prompt = (a:cmd == '/')? '/' : '?'
		" let prompt = (a:cmd == '/')? '(search): ' : '(search backwards): '
		let t = preview#core#input(prompt, '')
		if t != '' && t != "\<c-c>"
			try
				silent call preview#core#win_execute(a:winid, a:cmd . t)
			catch /^Vim\%((\a\+)\)\=:E486:/
				call preview#utils#errmsg('E486: Pattern not find: '. t)
			endtry
			silent! call preview#core#win_execute(a:winid, 'nohl')
			call setwinvar(a:winid, '__tools_search_cmd', a:cmd)
			call setwinvar(a:winid, '__tools_search_key', t)
		endif
	elseif a:cmd == ':'
		let prompt = ':'
		" let prompt = '(goto): '
		let t = preview#core#input(prompt, '')
		if t != ''
			call preview#core#win_execute(a:winid, ':' . t)	
		endif
	endif
endfunc
...
" search next
"----------------------------------------------------------------------
function! preview#utils#search_next(winid, cmd)
	let prev_cmd = getwinvar(a:winid, '__tools_search_cmd', '')
	let prev_key = getwinvar(a:winid, '__tools_search_key', '')
	if prev_key != ''
		...
		try
			silent call preview#core#win_execute(a:winid, cmd . prev_key)
		catch /^Vim\%((\a\+)\)\=:E486:/
		endtry
		noautocmd call preview#core#win_execute(a:winid, 'nohl')
	endif
endfunc
...
"----------------------------------------------------------------------
function! preview#utils#read_size(text, maxsize)
...
"----------------------------------------------------------------------
function! preview#utils#tools_width()
	let width = get(g:, 'tools_tools_width', '60%')
	let size = preview#utils#read_size(width, &columns)
